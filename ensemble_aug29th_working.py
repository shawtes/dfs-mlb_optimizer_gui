...
selected_features = [
    ...
    'singles', 'wOBA_Statcast', 'SLG_Statcast'
    # , 
...
def create_synthetic_rows(df, current_date):
    ...
    for player_id in df['Name'].unique():
        player_df = df[df['Name'] == player_id].tail(20)
        if player_df.empty:
            continue
...
# ]
    ...
    numeric_features = [
        'wOBA', 'BABIP', 'ISO',  'wRAA', 'wRC', 'wRC+', 'flyBalls', 'year', 'month', 'day',
        'rolling_min_fpts_7', 'rolling_max_fpts_7', 'rolling_mean_fpts_7', 'rolling_mean_fpts_49', 'wOBA_Statcast',
        'SLG_Statcast', 'RAR_Statcast', 'Offense_Statcast', 'Dollars_Statcast', 'WPA/LI_Statcast',
    ...
    for fold, (train_index, test_index) in enumerate(tscv.split(features), 1):
        ...
        # Perform feature engineering only on training data
        X_train = engineer_features(X_train.copy(), date_train)
        X_test = engineer_features(X_test.copy(), date_test)  # Apply same transformations to test data
        ...
        print(f'Fold {fold} - MAE: {mae}, MSE: {mse}, R2: {r2}, MAPE: {mape}')

        # Create a DataFrame with predictions, actual values, names, and dates
        results_df = pd.DataFrame({
            'Name': df.iloc[test_index]['Name'],
